...
await p.waitForSelector('.cmc__hit', { timeout: 120000 }); await p.waitForTimeout(2500)
console.log('6. unknown id → list shown:', await p.locator('.cmc__hit').count() > 0, '| url:', p.url().replace(/^https?:\/\/[^/]+/, ''))
