...
// ─────────────────────────────────────────────────────────────────────────────
// CANONICAL THREAD STATE MODEL (universal lead state registry)
// Stage = 10 lifecycle stages | Status = 9 operational statuses | Temperature = 4
// ─────────────────────────────────────────────────────────────────────────────

export type ThreadStatus = OperationalStatusCode
export type ThreadStage = LifecycleStageCode
export type ThreadTemperature = LeadTemperatureCode
...
const pillFromColor = (color: string): Pick<PillVisual, 'bg' | 'border'> => ({
  bg: `color-mix(in srgb, ${color} 12%, transparent)`,
  border: `color-mix(in srgb, ${color} 30%, transparent)`,
})

export const threadStatusVisuals: Record<ThreadStatus, PillVisual> = Object.fromEntries(
  OPERATIONAL_STATUS_ORDER.map((code) => {
    const meta = OPERATIONAL_STATUS_META[code]
    const extras = pillFromColor(meta.color)
    return [code, { label: meta.label, color: meta.color, ...extras }]
  }),
) as Record<ThreadStatus, PillVisual>

export const threadStageVisuals: Record<ThreadStage, PillVisual> = Object.fromEntries(
  LIFECYCLE_STAGE_ORDER.map((code) => {
...
) as Record<ThreadStage, PillVisual>

export const threadTemperatureVisuals: Record<ThreadTemperature, PillVisual> = Object.fromEntries(
  LEAD_TEMPERATURE_ORDER.map((code) => {
    const meta = LEAD_TEMPERATURE_META[code]
    const extras = pillFromColor(meta.color)
    return [code, { label: meta.label, color: meta.color, ...extras }]
  }),
) as Record<ThreadTemperature, PillVisual>
...
export const resolveThreadStatus = (thread: {
  inboxStatus?: string
  status?: string
...
}): ThreadStatus => normalizeOperationalStatus(
  thread.operational_status
  || thread.operationalStatus
  || thread.conversation_status
  || thread.conversationStatus
  || thread.inboxStatus
  || thread.status,
)
...
}): ThreadStage => normalizeLifecycleStage(
  thread.lifecycle_stage
  || thread.lifecycleStage
...
export const resolveThreadTemperature = (thread: {
  ...
  const normalized = normalizeLeadTemperature(
    thread.lead_temperature || thread.leadTemperature || thread.temperature,
  )
  if (normalized !== 'unscored') return normalized
