...
import { nexusGlobalThemes } from '../../modules/theme/nexusThemes'
import type { NexusGlobalThemeId } from '../../modules/theme/nexusThemes'
...
function resolveMapThemeId(nexusTheme: string): CommandMapThemeId {
  ...
  // Accept CommandMapThemeId values directly (e.g. from InboxCommandMap's activeThemeRef)
...
export function getMapThemeTokens(nexusTheme: string): PropertyLayerTokens {
  ...
  return {
    ...
    // Suppressed/not-contacted: bright enough to see, dim enough to be background
    colorSuppressed:   (pp.suppressed    as string | undefined) ?? '#7a8fa8',
    colorBuyerComp:    theme.buyerAccent,
    colorSoldComp:     theme.soldCompColor,
    colorNotContacted: (pp.not_contacted as string | undefined) ?? '#7a8fa8',
    colorBase:         isLight ? '#5a6b7e' : '#7a8fa8',
  }
}
