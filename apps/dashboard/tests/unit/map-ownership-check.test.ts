...
describe('map ownership check canonical resolver', () => {
  ...
  it('uses assigned agent names per master owner', async () => {
    ...
    expect(david.identity.smsAgentId).toBeNull()
    expect(david.identity.selectedAgentId).toBeNull()
    expect(anthony.identity.smsAgentId).toBeNull()
    expect(anthony.identity.selectedAgentId).toBeNull()
  })
