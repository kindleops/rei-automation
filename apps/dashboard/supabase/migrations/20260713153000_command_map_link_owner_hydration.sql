-- description: Hydrate map seller owner/phone/prospect from map_filter_property_prospect_links
-- when properties.master_owner_id is null, so uncontacted LLC pins can send ownership checks.
...
SELECT
  p.property_id,
  COALESCE(p.master_owner_id, pl.master_owner_id) AS master_owner_id,
  ...
  COALESCE(lt.prospect_id, bp.prospect_id, pl.prospect_id) AS prospect_id,
  COALESCE(bp.full_name, bp.first_name, mo.display_name) AS prospect_full_name,
...
LEFT JOIN LATERAL (
  SELECT
    l.master_owner_id,
    l.prospect_id
  FROM public.map_filter_property_prospect_links l
  WHERE l.property_id = p.property_id::text
...
LEFT JOIN public.master_owners mo
  ON mo.master_owner_id = COALESCE(p.master_owner_id, pl.master_owner_id)
LEFT JOIN LATERAL (
  SELECT
    pr.prospect_id,
    pr.full_name,
  ...
  WHERE pr.master_owner_id = COALESCE(p.master_owner_id, pl.master_owner_id)
  ORDER BY
    CASE WHEN pl.prospect_id IS NOT NULL AND pr.prospect_id = pl.prospect_id THEN 0 ELSE 1 END,
    pr.sms_eligible DESC,
...
SELECT
  ...
  nullif(mo.primary_phone_id, '') AS phone_id
