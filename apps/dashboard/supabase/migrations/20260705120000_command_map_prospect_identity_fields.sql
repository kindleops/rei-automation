-- description: Expose the linked prospect's own identity/eligibility fields, plus
-- the Master Owner's assigned SMS agent signal, on v_command_map_seller_pin_feed.
--
-- Without prospect_full_name/prospect_first_name/sms_eligible, the dashboard's Map
-- card had no way to resolve a real prospect first/full name or SMS eligibility and
-- fell back to treating every uncontacted pin as unpersonalizable ("Ownership check
-- template unavailable") even when a valid, SMS-eligible prospect (e.g. property
-- 274564949 / Amanda L Tallen) is linked. seller_name/seller_display_name on this
-- view already coalesce down to master_owners.display_name, which is exactly the
-- entity-name fallback the recipient-identity fix intentionally avoids using for
-- personalization — so this migration adds separate, prospect-only columns instead
-- of reusing those.
--
-- agent_persona/agent_family expose the Master Owner's loose sender-agent signal
-- (the same fields apps/api's outbound feeder already resolves agent_first_name
-- from) so the Map's ownership-check send can stop hardcoding "Chris" as the
-- sender name.
--
-- This is additive only: reproduces the full 20260704120000 view definition and
-- appends prospect_full_name / prospect_first_name / sms_eligible / agent_persona /
-- agent_family at the end.
...
SELECT
  ...
  -- Prospect-only identity/eligibility fields, deliberately NOT coalesced down to
  -- mo.display_name (Master Owner). A null here means "no linked prospect name/
  -- eligibility known" and callers must treat that as unresolved, not substitute
  ...
  -- Loose assigned-SMS-agent signal (same source apps/api's outbound feeder already
  -- resolves agent_first_name from). A null here means no agent is assigned yet —
  -- callers must fail closed / resolve an eligible agent, never invent a name.
