-- Fix ambiguity and parameter naming for ownership_check template analytics
-- Consolidates fixes for the RPC and the View.

-- 1. Drop existing functions to allow signature change
DROP FUNCTION IF EXISTS public.get_ownership_check_template_stats(timestamptz, timestamptz, text, text, text, integer);
DROP FUNCTION IF EXISTS public.get_ownership_check_template_stats_v2(timestamptz, timestamptz, text, text, text, integer, text, text);

-- 2. Recreate get_ownership_check_template_stats with p_ prefixes
...
BEGIN
  ...
  enriched_messages AS (
    ...
    WHERE (p_start_date IS NULL OR COALESCE(m.event_timestamp, m.created_at) >= p_start_date)
      AND (p_end_date IS NULL OR COALESCE(m.event_timestamp, m.created_at) <= p_end_date)
      AND (p_market IS NULL OR COALESCE(m.market_id, q.market_id) = p_market)
...
-- 3. Recreate ownership_check_template_kpis View
CREATE OR REPLACE VIEW public.ownership_check_template_kpis AS
SELECT * FROM public.get_ownership_check_template_stats();

-- 4. Recreate get_ownership_check_template_stats_v2 with p_ prefixes for consistency
