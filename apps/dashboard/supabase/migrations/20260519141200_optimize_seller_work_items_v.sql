-- description: Optimize seller work items view and command map seller pin feed view using lateral joins.

BEGIN;

DROP VIEW IF EXISTS public.v_seller_work_items CASCADE;

CREATE VIEW public.v_seller_work_items AS
SELECT
  p.property_id,
  p.master_owner_id,
...
-- Recreate the command map seller pin feed view
...
-- Recreate the get_command_map_seller_pins function
