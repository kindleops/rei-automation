...
const dimensions = [
  { view: 'template_performance_kpis_v', group_by: 'template_key' },
  { view: 'number_performance_kpis_v', group_by: 'textgrid_number_key', extra: ', MAX(o.from_phone_number) as from_phone_number, MAX(o.market) as market' },
  { view: 'market_performance_kpis_v', group_by: 'market' },
  { view: 'property_type_performance_kpis_v', group_by: 'property_type' },
  { view: 'seller_signal_performance_kpis_v', group_by: 'seller_signal' },
  { view: 'property_signal_performance_kpis_v', group_by: 'podio_tags' }, // Since property_signals_text is long text, we can use podio_tags or a specific signal. Let's use property_type for now or maybe property_signals_text if it's enum-like
  { view: 'owner_type_performance_kpis_v', group_by: 'owner_type' },
  { view: 'stage_performance_kpis_v', group_by: 'current_stage', extra: ', MAX(o.stage_before) as stage_before, MAX(o.stage_after) as stage_after' },
  { view: 'touch_performance_kpis_v', group_by: 'touch_number' },
  { view: 'language_performance_kpis_v', group_by: 'language' }
];
...
${dimensions.map(d => `DROP VIEW IF EXISTS public.${d.view} CASCADE;`).join('\n')}
...
for (const d of dimensions) {
  sql += `
CREATE OR REPLACE VIEW public.${d.view} AS
WITH windows AS (
...
WHERE 
  ...
  END
GROUP BY o.${d.group_by}, w.time_window;
`;
}
...
fs.writeFileSync('supabase/migrations/20260511000006_performance_intelligence_views.sql', sql);
