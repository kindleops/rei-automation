import { getSupabaseAdminClient } from '../api/internal/_lib/supabaseAdmin.js'
...
async function backfill() {
  ...
  for (const item of sentItems) {
    ...
    const { data: eventData, error: eventError } = await supabase.from('message_events').upsert({
      ...
      seller_display_name: item.seller_display_name || item.seller_first_name,
      market: item.market,
    ...
    if (eventError) {
      ...
      if (item.thread_key) {
        await supabase.from('inbox_thread_state').upsert({
          thread_key: item.thread_key,
          latest_message_event_id: eventData.id,
