...
// ── Regression ledger: the two expectations the temperature ruling retired ──

test("stage depth alone never produces HOT, at any stage, without economics", () => {
  // The retired rule was `afterIdx >= 4 -> HOT`, which made depth in the
  // funnel look like qualification. Two fixtures in the resolver matrix
  // encoded it. This proves the replacement holds across the whole ladder.
  for (const stage of ["offer_interest", "asking_price", "property_condition", "offer"]) {
    const t = resolveSellerStageTransition({
      stage_before: stage,
    ...
    assert.notEqual(t.lead_temperature, "hot", `${stage} must not be HOT without economics`);
  }
});

test("the SAME ask becomes HOT once economics say it is actionable", () => {
  // The other half of the rule: WARM is not a ceiling, it is the honest answer
  // when economics are unknown. Supply them and the seller qualifies.
  const priced = (ade) =>
    resolveSellerStageTransition({
      stage_before: "asking_price",
      known_facts: { ownership_status: "confirmed", interest: "interested" },
      new_facts: { asking_price: { value: 195_000, raw: "195k" } },
      intent: "price_provided",
      ade_result: { ...ade, sufficient_facts: true },
    });
  assert.equal(priced({ recommended_cash_offer: 200_000, max_allowable_offer: 230_000 }).lead_temperature, "hot");
  assert.equal(priced({ recommended_cash_offer: 100_000, max_allowable_offer: 115_000 }).lead_temperature, "cold");
});

test("a seller asking to transact is HOT on priority, without advancing stage", () => {
  // Temperature is acquisition PRIORITY. A contract request is the most
  // actionable state short of signed - but it raises neither the stage nor
  // terms_accepted, which remain the route's and the acceptance resolver's.
  const t = resolveSellerStageTransition({
    stage_before: "offer",
    known_facts: { ownership_status: "confirmed", interest: "interested" },
    new_facts: {},
  ...
  assert.equal(t.lead_temperature, "hot");
  assert.equal(t.stage_after, "offer", "priority is not stage");
