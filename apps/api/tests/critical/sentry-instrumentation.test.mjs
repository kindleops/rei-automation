...
test("captureRouteException: still captures unexpected provider transport failures", () => {
  const mock = makeMockSentry();
  __setSentryDeps({ sentry: mock });

  try {
    const error = new Error("TextGrid HTTP failure: upstream timeout after 30s");
    error.name = "TextGridError";

    assert.equal(shouldCaptureRouteException(error), true);
    captureRouteException(error, {
      route: "internal/queue/run",
      subsystem: "queue_runner",
    });
