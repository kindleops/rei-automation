...
export async function getUniversalDealDossier({
  ...
  if (resolved_property_id && resolved_prospect_id && resolved_canonical_e164) {
    const { data } = await supabase.from('v_universal_lead_command').select(COMMAND_COLUMNS)
  ...
  if (!command && thread_key) {
    const { data } = await supabase.from('v_universal_lead_command').select(COMMAND_COLUMNS)
      .eq('thread_key', thread_key).limit(1).maybeSingle()
    command = data
  }
  ...
  if (resolved_canonical_e164) {
    const { data: smsSupp } = await supabase
  ...
  const dossier = {
    ...
    phones: command?.contact_threads?.filter(t => t.channel === 'phone') || [],
    primary_phone: {
    ...
    emails: command?.contact_threads?.filter(t => t.channel === 'email') || [],
    primary_email: {
