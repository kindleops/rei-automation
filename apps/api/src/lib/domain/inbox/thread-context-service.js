...
export async function loadThreadContext({ thread_key, supabase }) {
  ...
  const [
    ...
    safeSelect(supabase, "properties", (q) => (propertyIds.length ? q.select("property_id,master_owner_id,owner_id,property_address,property_address_full,property_address_city,property_address_state,property_address_zip,market,property_type,beds,baths,sqft,year_built,estimated_value,arv,motivation_score,priority_score,status").in("property_id", propertyIds) : Promise.resolve({ data: [] }))),
    safeSelect(supabase, "master_owners", (q) => (masterOwnerIds.length ? q.select("*").in("master_owner_id", masterOwnerIds) : Promise.resolve({ data: [] }))),
    safeSelect(supabase, "owners", (q) => (ownerIds.length ? q.select("*").in("owner_id", ownerIds) : Promise.resolve({ data: [] }))),
    safeSelect(supabase, "prospects", (q) => (ownerIds.length ? q.select("*").in("prospect_id", ownerIds) : Promise.resolve({ data: [] }))),
