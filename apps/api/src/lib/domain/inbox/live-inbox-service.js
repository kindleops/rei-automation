...
export async function getLiveInbox(params = {}, deps = {}) {
  ...
  if (threadKeys.length > 0) {
    ...
    let eventsQuery = supabase.from('message_events').select('*');
    if (typeof eventsQuery.in === 'function') {
      eventsRes = await eventsQuery.in('thread_key', threadKeys);
    } else {
      eventsRes = await eventsQuery;
    }
    events = eventsRes.data || [];

    let queueQuery = supabase.from('send_queue').select('*');
    if (typeof queueQuery.in === 'function') {
      queueRes = await queueQuery.in('thread_key', threadKeys);
    } else {
      queueRes = await queueQuery;
    }
    queue = queueRes.data || [];
    ...
    let contextQuery = supabase.from('deal_context_index').select('*');
    if (typeof contextQuery.or === 'function') {
      const orClauses = [];
      if (threadKeys.length) orClauses.push(`thread_key.in.(${threadKeys.map(k => `"${k}"`).join(',')})`);
      if (propertyIds.length) orClauses.push(`property_id.in.(${propertyIds.map(id => `"${id}"`).join(',')})`);
      if (masterOwnerIds.length) orClauses.push(`master_owner_id.in.(${masterOwnerIds.map(id => `"${id}"`).join(',')})`);
      
      if (orClauses.length) {
        const { data } = await contextQuery.or(orClauses.join(','));
        contextData = data || [];
      }
    } else {
  ...
  if (typeof query.or !== 'function') {
    if (filter === 'dead') {
      ...
      finalRows = enrichedRows.filter(r => r.inbox_bucket === 'needs_review' || r.needs_review);
    ...
    if (params.q) {
      ...
      finalRows = finalRows.filter(r => 
        (r.message_body && r.message_body.toLowerCase().includes(qLower)) ||
        (r.thread_key && r.thread_key.toLowerCase().includes(qLower))
      );
    }
  }

  const liveCounts = await getLiveCounts({}, deps);
