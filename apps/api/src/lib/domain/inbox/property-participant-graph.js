...
export function normalizePropertyParticipantRow(row = {}) {
  return {
    ...
    ownership_status: row.ownership_status || "unconfirmed",
    ownership_confidence:
      typeof row.ownership_confidence === "number" ? row.ownership_confidence : null,
    ownership_source: row.ownership_source || null,
    ownership_inference_reason: row.ownership_inference_reason || null,
    owner_match_flags: Array.isArray(row.owner_match_flags) ? row.owner_match_flags : [],
    contact_rank: Number.isFinite(Number(row.contact_rank)) ? Number(row.contact_rank) : null,
    contact_rank_label: row.contact_rank_label || null,
    contact_score: Number.isFinite(Number(row.contact_score)) ? Number(row.contact_score) : null,
    best_phone_score: Number.isFinite(Number(row.best_phone_score)) ? Number(row.best_phone_score) : null,
    sms_eligible: row.sms_eligible !== false,
    contactability: row.contactability || (row.safe_to_contact === false ? "blocked" : "contactable"),
    likely_owner: row.likely_owner === true,
    likely_renting: row.likely_renting === true,
    matching_flags: row.matching_flags || null,
    person_flags_text: row.person_flags_text || null,
    excluded_as_renter: Boolean(row.excluded_as_renter),
    needs_review: Boolean(row.needs_review),
    active_thread_state: row.active_thread_state || null,
