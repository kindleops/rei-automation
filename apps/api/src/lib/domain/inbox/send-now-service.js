 ...
 *
 * Checks (in order):
 *   a) Thread state — blocks paused_review and quarantine (fail-open: DB error ≠ hard block)
 *   b) Suppression — sms_suppression_list, fail-CLOSED (DB error → block; TCPA asymmetry)
...
export async function canSend(payload = {}, deps = {}) {
  ...
  // b) Suppression — single authority (Rule 11). Fails CLOSED: TCPA risk
  //    is asymmetric — a false block is recoverable; a false send is not.
  if (to_phone) {
    try {
