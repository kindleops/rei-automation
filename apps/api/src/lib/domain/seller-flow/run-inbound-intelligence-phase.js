...
const INTELLIGENCE_DECISION_VERSION = "inbound_intelligence_v2_shadow";

function clean(value) {
  return String(value ?? "").trim();
}

function deriveUniversalStage(decision = {}, route = null, context = null) {
  const route_stage = clean(route?.stage) || clean(context?.summary?.conversation_stage);
  if (route_stage) return route_stage;
  if (decision?.route_hint) return decision.route_hint;
  return SELLER_FLOW_STAGES.OWNERSHIP_CHECK;
}

function deriveGranularStage(decision = {}, template = null, route = null) {
  return (
...
function applyRelationshipOverride(canonical_decision = {}, relationship = null) {
  ...
  if (relationship.is_property_scoped || relationship.referral_detected) {
...
export async function runInboundIntelligencePhase({
  ...
  const canonical_decision = applyRelationshipOverride(raw_canonical_decision, relationship);
  ...
  const template_result = relationship.is_property_scoped
  ...
  const intelligence_snapshot = {
    ...
    universal_stage: deriveUniversalStage(canonical_decision, route, latestThreadContext),
    granular_stage: deriveGranularStage(canonical_decision, template_result?.template, route),
  ...
  info("[INBOUND_INTELLIGENCE_PHASE]", {
    ...
    shadow_canonical_agreement: shadow_stage.canonical_agreement,
  });
