...
export function buildCanonicalLiveCampaignPatch(campaign = {}, schedule = {}) {
  ...
  const executionMode = isFutureSchedule ? 'scheduled_live' : 'immediate_live'
  return {
    ...
    metadata: {
      ...
      // Phase 2: the activation transaction must clear every proof marker so the
      // campaign can never remain active while advertising proof/no-send state.
