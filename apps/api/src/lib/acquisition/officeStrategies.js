/**
 * Acquisition Engine V3 — Item 5F §20–§22: office acquisition strategies +
 * class-first qualification.
 *
 *   - Cash: full dollar bridge anchored on the conservative office-buyer exit,
 *     including repairs, TI/LC, rollover, downtime, operating deficit, diligence,
 *     environmental, legal, financing, holding, contingency and dynamic margin.
 ...
 *   - Office marketed disposition: a commercial brokered sale; NOT residential
 *     novation.
 *   - Owner-user disposition: a SEPARATE strategy, evaluated only with real owner-
 *     user demand; it never sets the investor exit.
 *
 * Each strategy carries its OWN qualification state with hardened authorization
 * semantics (preflight §0): a non-EXECUTABLE status is a LIVE-EXECUTION gate, never
 * an "economics failed" signal. AUTO / live authorization stay disabled. Pure &
 * deterministic.
 */
...
import { OFFICE_DISPOSITION_STRATEGY, OFFICE_OWNER_USER_DISPOSITION_STRATEGY, OFFICE_DEBT_MODEL } from './officeConstants.js';

/* -------------------------------------------------------------------------- */
/* Cash (§20)                                                                  */
/* -------------------------------------------------------------------------- */

export function buildOfficeCashOffer({ buyerExit, capital, rollover, noi, operationalStatus, environmental = false, demand = 50, confidence = 0 }) {
  ...
  const dueDiligence = exit * 0.007;
  const environmentalCost = environmental ? 12_000 : 5_000; // Phase I (higher for older/medical exposure)
  const legal = exit * 0.005;
  const financing = exit * 0.018;
  // Operating-deficit / stabilization loss for value-add / lease-up while re-tenanting.
  const currentNoi = num(noi?.current_noi?.noi);
  const stabilizedNoi = num(noi?.stabilized_noi?.noi);
  const operatingDeficit = (operationalStatus === 'VALUE_ADD' || operationalStatus === 'LEASE_UP') && currentNoi !== null && stabilizedNoi !== null && stabilizedNoi > currentNoi
    ? (stabilizedNoi - currentNoi) * 1.75 // office downtime is longer than retail
    : 0;
  ...
  const deductions = oneTimeCapital + leasingCapital + closing + holding + disposition + contingency + dueDiligence + environmentalCost + legal + financing + operatingDeficit + margin;
  ...
  const bridge = [
    ...
    { line: 'operating_deficit_during_leaseup', amount: -roundMoney(operatingDeficit) },
...
export function buildOfficeSellerFinance({ valuation, noi, rollover, capitalRequired = 0 }) {
...
export function buildOfficeCommercialDebt({ contract, noi }) {
...
/* -------------------------------------------------------------------------- */
/* Office marketed disposition (§20) — NOT residential novation                 */
/* -------------------------------------------------------------------------- */

export function buildOfficeDisposition({ valuation, buyerExit, cashRecommended }) {
  const value = num(valuation?.reconciliation?.reconciled_value_mid);
  if (value === null) {
    return { strategy: OFFICE_DISPOSITION_STRATEGY, is_residential_novation: false, qualification: SQ.DATA_REQUIRED, available: false, reason: 'no_value_basis' };
  }
  const brokerage = value * 0.04;
  ...
  return {
    strategy: OFFICE_DISPOSITION_STRATEGY,
    ...
    note: 'Office marketed disposition — a commercial brokered sale; NOT residential novation.',
  };
}

/* -------------------------------------------------------------------------- */
/* Owner-user disposition (§20) — separate; only with real owner-user demand     */
/* -------------------------------------------------------------------------- */

export function buildOwnerUserDisposition({ ownerUser, buyerExit }) {
  const value = num(ownerUser?.owner_user_value);
  if (!ownerUser?.available || value === null) {
    return { strategy: OFFICE_OWNER_USER_DISPOSITION_STRATEGY, is_residential_novation: false, qualification: SQ.DATA_REQUIRED, available: false, reason: 'no_owner_user_value' };
  }
  const demandConfirmed = ownerUser.owner_user_demand_confirmed === true;
  const brokerage = value * 0.04;
  const closing = value * 0.015;
  const sellerNet = roundMoney(value - brokerage - closing);
  // Without confirmed owner-user demand the strategy is provisional, never shadow.
  const qualification = demandConfirmed ? SQ.PROVISIONAL_SCENARIO : SQ.PROVISIONAL_SCENARIO;
  return {
    strategy: OFFICE_OWNER_USER_DISPOSITION_STRATEGY,
    is_residential_novation: false,
    qualification,
    available: true,
    authorized_offer: false,
    owner_user_value: value,
    projected_seller_net: sellerNet,
    owner_user_demand_confirmed: demandConfirmed,
    sets_investor_exit: false, // explicit invariant
    note: 'Owner-user disposition — a separate universe; evaluated only with real owner-user demand and never sets the investor exit.',
  };
}
...
function extractOfferFigures(entry) {
  ...
  if (entry.strategy === OFFICE_DISPOSITION_STRATEGY) {
...
/**
 * Derive the full, unambiguous semantic-flag set for a single office strategy.
 * Mirrors the hardened storage/retail semantics: a non-EXECUTABLE status is a
 * LIVE-EXECUTION gate (`live_execution_not_authorized`), NOT an economic-gate
 * failure — so shadow approval is never conflated with live authorization.
 */
...
/**
 * Class-first office strategy qualification. Office SHADOW_MODE_READY requires a
 * reliable office classification, rentable area, sufficient compatible pricing OR
 * income evidence, a supportable buyer exit, capital + rollover costs and the
 * invariants. Income-led strategies additionally require lease/rent, occupancy,
 * expenses, NOI and cap-rate/DCF support. Medical-led strategies additionally
 * require enough evidence to support medical use. AUTO / live authorization stay
 * disabled.
 */
export function qualifyOfficeStrategies({
  classification, contract, valuation, noi, revenue, capRate, rollover, buyerExit, capital,
  strategies, recordGated = false, medicalEvidenceOk = true, liveFlagsEnabled = false,
}) {
  const buyerExitQualified = buyerExit?.exit_classification === 'QUALIFIED';
  const requirements = {
    asset_classification: Boolean(classification?.is_office && classification?.genuine_office) && !recordGated,
    reliable_size: Boolean(num(contract?.physical?.rentable_building_area?.value)),
    ...
    medical_evidence: contract?.is_medical ? medicalEvidenceOk : true,
    invariants_ok: true,
  };
  ...
  const classFirstOk = !recordGated && requirements.asset_classification && requirements.reliable_size &&
    requirements.transaction_or_income_evidence && requirements.medical_evidence;
  ...
  const evaluate = (entry) => {
    ...
    const isIncomeLed = entry.strategy === S.SELLER_FINANCE || entry.debt_model === OFFICE_DEBT_MODEL;
  ...
  evaluate(strategies.owner_user_disposition);
  ...
  return {
    ...
    medical_evidence_ok: requirements.medical_evidence,
    ranked,
 ...
 */
export function buildOfficeExecutionBasis({ ranked = [], executionState, liveFlagsEnabled = false }) {
