...
import { createEdge } from '@/lib/domain/workflow-v2/graph-service.js';
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const result = await createEdge(id, payload);
    return withCors(request, result, result.ok === false ? Number(result.status ?? 400) : 201);
  } catch (error) {
    return withCors(request, { ok: false, error: 'wfv2_edge_create_failed', message: error?.message ?? String(error) }, 500);
  }
}
