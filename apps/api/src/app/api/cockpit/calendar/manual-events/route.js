...
} from '@/lib/domain/calendar/calendar-nexus-service.js';
...
export async function PATCH(request) {
  ...
  try {
    const payload = await parseJsonSafe(request);
    const result = await updateManualCalendarEvent(payload);
    const status = result.ok ? 200 : 400;
    return NextResponse.json(result, { status, headers });
  } catch (error) {
    return NextResponse.json(
      { ok: false, error: error?.message || 'manual_event_update_failed' },
      { status: 500, headers },
    );
  }
}
...
export async function DELETE(request) {
  ...
  try {
    const payload = await parseJsonSafe(request);
    const result = await deleteManualCalendarEvent(payload);
    const status = result.ok ? 200 : 400;
    return NextResponse.json(result, { status, headers });
  } catch (error) {
    return NextResponse.json(
      { ok: false, error: error?.message || 'manual_event_delete_failed' },
      { status: 500, headers },
    );
  }
}