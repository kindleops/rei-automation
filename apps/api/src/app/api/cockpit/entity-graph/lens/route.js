...
import { getEntityGraphLens } from '@/lib/domain/entity-graph/entity-graph-service.js'
...
export async function GET(request) {
  ...
  try {
    ...
    const lens = await getEntityGraphLens(params)
    return NextResponse.json({ ok: true, lens }, { status: 200, headers })
  } catch (error) {
    return NextResponse.json(
      { ok: false, error: error?.message || 'entity_graph_lens_failed' },
      { status: 500, headers },
    )
  }
}
