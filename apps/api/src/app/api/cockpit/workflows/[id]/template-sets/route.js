...
export async function GET(request, { params }) {
  ...
  try {
    ...
    const result = await listWorkflowTemplateSets(resolved?.id);
    return withCors(request, result, result.ok === false ? Number(result.status || 400) : 200);
  } catch (error) {
    return withCors(request, {
      ok: false,
      error: "workflow_template_sets_list_failed",
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const result = await createWorkflowTemplateSet(resolved?.id, payload);
    return withCors(request, result, result.ok === false ? Number(result.status || 400) : 200);
  } catch (error) {
    return withCors(request, {
      ok: false,
      error: "workflow_template_set_create_failed",
      message: error?.message || String(error),
    }, 500);
  }
}
