-- Align canonical_inbox_counts new_replies with JS bucket predicates (exclusive precedence).
...
SELECT
  ...
  (
    COUNT(*) FILTER (WHERE inbox_bucket = 'priority')
    + (SELECT COUNT(*) FROM new_replies_facts)
    + COUNT(*) FILTER (WHERE inbox_bucket = 'needs_review' OR needs_review = true)
    + COUNT(*) FILTER (WHERE inbox_bucket = 'follow_up')
  ) AS active,
  ...
  (SELECT COUNT(*) FROM new_replies_facts) AS new_inbound,
  (SELECT COUNT(*) FROM new_replies_facts) AS needs_reply,
