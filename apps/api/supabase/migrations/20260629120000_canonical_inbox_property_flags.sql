-- Expose property distress flags on canonical_inbox_threads for inbox list badges.
...
SELECT
  ...
  CASE WHEN ts.latest_direction = 'inbound' THEN NULL ELSE ts.latest_delivery_status END AS delivery_status,
  CASE WHEN ts.latest_direction = 'inbound' THEN NULL ELSE ts.latest_delivery_status END AS latest_delivery_status,
  CASE WHEN ts.latest_direction = 'inbound' THEN NULL ELSE ts.latest_delivery_status END AS latest_provider_delivery_status,
  ...
  COALESCE(
    NULLIF(p.property_flags_text, ''),
    NULLIF(p.seller_tags_text, ''),
    NULLIF(mo.seller_tags_text, ''),
    NULLIF(p.podio_tags, '')
  ) AS property_flags_text,
  p.property_flags_json,
  COALESCE(p.tax_delinquent, false) AS tax_delinquent,
  COALESCE(p.active_lien, false) AS active_lien,
  CASE
    WHEN ts.is_hot_lead = true OR ts.is_urgent = true THEN 'priority'
    WHEN lower(COALESCE(ts.inbox_bucket, '')) = 'priority' THEN 'priority'
    WHEN lower(COALESCE(ts.inbox_bucket, '')) = 'new_replies' THEN 'new_replies'
    WHEN lower(COALESCE(ts.inbox_bucket, '')) = 'needs_review' THEN 'needs_review'
    WHEN lower(COALESCE(ts.inbox_bucket, '')) = 'waiting' THEN 'waiting'
    WHEN lower(COALESCE(ts.inbox_bucket, '')) = 'follow_up' THEN 'follow_up'
    WHEN lower(COALESCE(ts.inbox_bucket, '')) = 'cold' THEN 'cold'
    ELSE NULL
    ...
    WHERE inbox_bucket NOT IN ('dead', 'suppressed')
      AND (
        ...
        OR (
          ...
          AND COALESCE(latest_delivery_status, '') IN ('sent', 'delivered', 'accepted')
        )
      )
  ) AS waiting,
